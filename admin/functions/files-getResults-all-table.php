        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                if($row["nvchdescripcion"] == null){
                    $row["nvchdescripcion"] = 'Sin descripción';
                }

                echo '
                  <tr>
                    <td>'.$row["intidcodigo"].'</td>
                    <td><p class="truncate">'.$row["nvchtitulo"].'</p></td>
                    <td>'.$row["nvchdescripcion"].'</td>
                    <td>'.$row["dtfecha"].'</td>
                    <td><a href="'.$row["nvcharchivo"].'" target="_blank">'.$row["nvcharchivo"].'</a></td>
                    <td>
                        ...
                        <a class="btn btn-danger btn-xs" onclick="'.$pregunta.'" href="functions/files-eliminarFile.php?id=7d7218764483c814ce24f&codid='.$row["intidcodigo"].'&file='.$row["nvcharchivo"].'&flag=7d72187644f348522423f83c814ce24f" >
